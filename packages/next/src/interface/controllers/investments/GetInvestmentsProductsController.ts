...
import type { GetInvestmentsProductsUseCase } from '../../../application/useCases';
import { EXECUTION_CONTEXT_TOKEN } from '../../../infra/di/tokens';
import { GET_INVESTMENTS_PRODUCTS_USE_CASE_TOKEN } from '../../../infra/di/generated/tokens';
...
import { GetInvestmentsProductsSchema } from './schemas';

/** Controller (manual, Parte 2.5.1) for `GET /api/investments/products`. */
@injectable()
export class GetInvestmentsProductsController {
  constructor(
    @inject(GET_INVESTMENTS_PRODUCTS_USE_CASE_TOKEN)
    private readonly getInvestmentsProductsUseCase: GetInvestmentsProductsUseCase,
  ...
  async handle(_request: Request): Promise<Response> {
    const log = buildLogContext(this.executionContext, 'getInvestmentsProducts');
    this.logger.info('request_start', log);

    try {
      const validation = GetInvestmentsProductsSchema.safeParse({
      ...
      const result = await this.getInvestmentsProductsUseCase.execute(validation.data);
      this.logger.info('getInvestmentsProducts_completed', log);
