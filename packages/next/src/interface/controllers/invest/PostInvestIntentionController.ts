...
import { PostInvestIntentionSchema } from './schemas';

/** Controller (manual, Parte 2.5.1) for `POST /api/invest/intention`. */
@injectable()
export class PostInvestIntentionController {
  ...
  async handle(request: Request): Promise<Response> {
    const log = buildLogContext(this.executionContext, 'postInvestIntention');
    this.logger.info('request_start', log);

    try {
      const body = await readJsonBody(request);
      const validation = PostInvestIntentionSchema.safeParse({
        ...body,
      ...
      const result = await this.postInvestIntentionUseCase.execute(validation.data);
      this.logger.info('postInvestIntention_completed', log);
