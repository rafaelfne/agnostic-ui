...
export function FlowEditorPage(): ReactElement {
  ...
  useEffect(() => {
    let active = true;
    client
      .listArtifacts('integration')
      .then((list) => {
        if (active) setIntegrations(list.map((a) => a.slug));
      })
      .catch(() => {
        /* opcional — sem catálogo, o chip cai no valor atual */
      });
    return () => {
      active = false;
    };
  ...
  return (
    <Tabs value={tab} onValueChange={setTab} className="flex min-h-full flex-col gap-0">
      ...
      <TabsContent value="editor" className="mx-auto w-full max-w-[1180px] px-8 pb-14 pt-6">
        ...
        {/* Config-como-frase — a superfície amigável (protótipo da direção). */}
        <div className="mb-4">
          <FlowSummaryPanel draft={draft} integrations={integrations} onChange={update} />
        </div>

        {/* Status amigável + toggle do Modo desenvolvedor. */}
        <div className="mb-4 flex flex-wrap items-center gap-3">
          {validation.ok ? (
            <span className="flex items-center gap-2 rounded-lg border border-success/30 bg-success/10 px-3.5 py-2 text-sm font-medium text-success">
              <CheckCircle2 className="size-4" /> {t('flow.status.ready')}
            </span>
          ) : (
            <button
              type="button"
              onClick={() => setDevMode(true)}
              className="flex items-center gap-2 rounded-lg border border-warning/40 bg-warning/10 px-3.5 py-2 text-sm font-medium text-warning"
            >
              <AlertCircle className="size-4" />{' '}
              {t('flow.status.issues', { n: validation.issues.length })}
            </button>
          )}
          <button
            type="button"
          ...
          >
            <span
              className={`relative h-5 w-9 rounded-full transition-colors ${devMode ? 'bg-primary' : 'bg-border'}`}
            >
              <span
                className={`absolute top-0.5 size-4 rounded-full bg-white shadow transition-transform ${devMode ? 'translate-x-4' : 'translate-x-0.5'}`}
              />
            </span>
            {t('flow.dev.toggle')}
          </button>
        ...
        {devMode && (
          <>
            <p className="mb-4 flex items-start gap-2 rounded-lg border bg-muted/40 px-3.5 py-2.5 text-xs text-muted-foreground">
              <span aria-hidden>⚙</span> {t('flow.dev.note')}
            </p>
